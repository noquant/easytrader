...
class TestUniversalClientTrader(unittest.TestCase):
    ...
    def setUpClass(cls):
        ...
        # input your test account and password
        cls._ACCOUNT = os.environ.get("EZ_TEST_UNIVERSAL_ACCOUNT") or "your account"
        cls._PASSWORD = os.environ.get("EZ_TEST_UNIVERSAL_PASSWORD") or "your password"
        cls._COMM_PASSWORD = (
            os.environ.get("EZ_TEST_UNIVERSAL_COMM_PASSWORD") or "your comm password"
        )

        cls._user = easytrader.use("universal_client")

        cls._user.prepare(
            user=cls._ACCOUNT, password=cls._PASSWORD, comm_password=cls._COMM_PASSWORD, exe_path="C:\\同花顺软件\\同花顺\\xiadan.exe"
        )
